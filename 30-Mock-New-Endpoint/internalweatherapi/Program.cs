internal class Program
{
    private static void Main(string[] args)
    {
        ...
        builder.Services.AddHttpClient("externalweatherapi", c => c.BaseAddress = new Uri("http://localhost:3003"));
        ...
        .WithOpenApi();
         ...
         app.MapGet("/sunriseForecast", () =>
        {
            //use the httpclient to call the external api
            var client = app.Services.GetRequiredService<IHttpClientFactory>().CreateClient("externalweatherapi");
            var response = client.GetAsync("/sunriseforecast").Result;
            var sunriseForecast = response.Content.ReadFromJsonAsync<SunriseForecast[]>().Result;
            return sunriseForecast;
        })
        .WithName("GetSunriseForecast")
        .WithOpenApi();
...
record SunriseForecast(DateOnly Date, int TemperatureC);